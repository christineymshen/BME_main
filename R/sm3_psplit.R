  ...
  runtime <- system.time({
    ...
    for (q in 1:nprior){
      ...
      dataset_spt <- list(n=data_spt_tr$n,p=data_spt_tr$p,k=k,m=data_spt_tr$m,y=data_spt_tr$data$time, s=s,
                          ...
                          tau0=taus[q])
      runres_spt <- sampling(mod,data=dataset_spt,chains=nchains,warmup=burnin,iter=niter,
                             cores=nchains,thin=thin,seed=slurm_id,refresh=0,
                             include=T,pars=c("beta","lambda"))
      
      p_beta_spt <- as.matrix(runres_spt,par="beta")
      p_lambda_spt <- as.matrix(runres_spt,par="lambda")

...
# prior
prior_labels <- c("good", "both bad", "bad mean")
nprior <- length(prior_labels)

# prior for mu
tau2s <- c(4, rep(4/p,2)) # used 4 instead of true para 1.5^2 because will set large prior for s2
mu0s <- sqrt(tau2s)*qnorm(0.95)* c(0,1,1)
...
a1s <- c(0,3,3) # 0 -> use half cauchy prior
b1s <- c(0,14,8) # 0 -> use half cauchy prior
...
calpepd <- function(y,X,p_m,p_b,p_alphas){
  ...
  log_diff_obs <- log(ratio_obs)
  prop_diff_obs <- ratio_obs-1
...
runtime <- system.time({
  ...
  for (q in 1:nprior){
    ...
    pepd_obs <- calpepd(y,X,p_m,p_b,p_alphas)
    ...
    pepd_rep <- foreach(i=1:nsim, .combine="rbind") %dopar% {
      ...
      pepd_rep_i <- calpepd(yrep[i,],X,p_m_i,p_b_i,p_alphas_i)

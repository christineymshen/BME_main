...
calpsplit <- function(){
  ...
  tmp <- t(sapply(c(1:nsim), function(i) prop_diff_rep[i,]%*%Xrep_spt[,,i]))
  Drep3_beta <- rowSums(tmp) * p_spt_alphas
  Drep[,3] <- -Drep[,1] + Drep3_beta
  
  # out of model score
  Dobs[,4] <- rowSums(prop_diff_obs * outer(nsim1, znew)) * p_spt_alphas
  Drep[,4] <- sapply(c(1:nsim), function(i) sum(prop_diff_rep[i,] * zrep_spt[,i])) * p_spt_alphas
...
runtime <- system.time({
  ...
  set.seed(1)
  Xrep_spt <- replicate(nsim, apply(Xobs, 2, sample, replace=T))
  zrep_spt <- replicate(nsim, sample(zobs,replace=T))
  ...
  for (q in 1:nprior){
    ...
    p_spt_m <- exp(t(sapply(c(1:nsim), function(i) Xrep_spt[,,i]%*%p_spt_betas[i,])))

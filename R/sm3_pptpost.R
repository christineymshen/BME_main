...
simnum <- 3
simlabel <- "sm"
p_label <- "pptpost" # p-value to be calculated
...
if (!file.exists(output_name) | override){
  ...
  runtime <- system.time({
    ...
    for (q in 1:nprior){
      ...
      ppost_obs <- calppost(data$data[ise,],data$delta[ise,],data_rep$res$time_rep_e,
      ...
      ppost_rep <- foreach(i=1:nsim, .combine="rbind") %dopar% {
        ...
        data_rep_i <- to_sim_ppdata2(data,s_i,p_beta_i,p_lambda_i,seed=slurm_id)
        
        ppost_i <- calppost(survdata_i,data_rep$res$delta_rep_e[i,,],data_rep_i$res$time_rep_e,
                            data_rep_i$res$delta_rep_e, s_i,p_beta_i,p_lambda_i,Xe)
        
        return(ppost_i)
      }
      
      output[q,] <- pr_ge(outer(one_nsim, ppost_obs),ppost_rep)
      # # for debugging

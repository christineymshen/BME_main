...
calpepd <- function(y,X,z,p_m,p_b,p_alphas){
...
runtime <- system.time({
  ...
  for (q in 1:nprior){
    ...
    p_m_betas <- colMeans(p_betas)
    p_m <- as.matrix(stan_mod, pars="m")
    p_b <- sweep(1/p_m, 1, p_alphas, "*")

    pepd_obs <- calpepd(y,X,z,p_m,p_b,p_alphas)
    ...
    pepd_rep <- foreach(i=1:nsim, .combine="rbind") %dopar% {
      ...
      pepd_rep_i <- calpepd(yrep[i,],Xrep[,,i],zrep[,i],p_m_i,p_b_i,p_alphas_i)
